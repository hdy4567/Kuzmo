...
export function initWorker() {
    state.worker = new Worker(new URL('../engine/ai_neural_worker.js', import.meta.url), { type: 'module' });
    ...
    state.worker.onmessage = async (e) => {
        ...
        if (action === 'AI_SMART_RESULT') {
            const { typeMessage } = await import('../engine/ai.js');
            const { filterByTag } = await import('../features/navigation.js');
            const { showDetailSheet } = await import('../view/ui/ui_base.js');
            ...
            if (contextId) {
                const event = state.eventStore.find(ev => ev.id === contextId);
                if (event) {
        ...
        if (action === 'AI_LABEL_RESULT' && e.data.results) {
            ...
            e.data.results.forEach(res => {
                const ev = state.eventStore.find(it => String(it.id) === String(res.id));
                if (ev) {
                    ...
                    // 🧠 [SOTA] Store neural embedding for high-D vector similarity
                    if (res.embedding) {
                        ev.embedding = res.embedding;
                    }
            ...
            saveToDB();
            import('../features/search.js').then(s => s.filterMarkers());
            ...
            if (!e.data.isAudit && state.autoLabelingEnabled) {
                import('../engine/ai.js').then(m => m.autoHealingAudit());
            }
        }
    };
}
...
export function updateLabelMonitor(...args) {
    import('../view/ui/ui_monitor.js').then(m => m.updateLabelMonitor(...args));
}

export function filterMonitor(...args) {
    import('../view/ui/ui_monitor.js').then(m => m.filterMonitor(...args));
}
