...
let perfThrottle = false; // ⚡ [PERF-THROTTLE] (v22.50)
...
self.onmessage = async function (e) {
    const { action, text, learningData, events, history, localTime, mode } = e.data;
    
    if (action === 'SET_PERF_MODE') {
        perfThrottle = (mode === 'CRITICAL' || mode === 'LOW_POWER');
        console.log(`🤖 [WORKER] Throttling mode: ${perfThrottle ? 'ENABLED' : 'DISABLED'}`);
        return;
    }
    ...
    if (action === 'AI_SMART_PROCESS' && text) {
        try {
            if (!extractor) {
                self.postMessage({ status: 'loading', message: `Neural Worker Standby...` });
            ...
            const topMatches = retrieved.sort((a,b) => b.sim - a.sim).filter(r => r.sim > 0.32);
            const matches = await matchCoreTag(queryVec, text);
            const region = matches.region ? matches.region.replace('@', '') : (topMatches[0]?.region || "주변");

            // 🚀 서버가 응답 중이라면 워커는 텍스트 답변을 하지 않고 '맵 이동/데이터 보조'만 수행
            const resultMsg = {
                success: true,
                action: 'AI_SMART_RESULT',
                response: e.data.hasServer ? null : `(로컬 모드) ${region} 근처에서 찾은 장소: ${topMatches[0]?.title || '검색 결과 없음'}`,
                location: `@${region}`,
                contextId: topMatches[0]?.id || null,
                coordinates: topMatches[0] ? [topMatches[0].lat, topMatches[0].lng] : null,
                quickActions: e.data.hasServer ? [] : ["📡 서버 연결 시도"]
            };
    ...
    if (action === 'AI_BATCH_LABEL' && events) {
        try {
            ...
            for (const ev of events) {
                // 🛑 [THROTTLE-SLEEP] CRITICAL 모드 시 GC 타임을 벌기 위해 주기적 지연
                if (perfThrottle) await new Promise(r => setTimeout(r, 150)); 
                try {
                    ...
                    labeledResults.push({
                        ...
                        embedding: Array.from(vec) // 🧠 [SOTA] Expose vector for HNSW/Similarity
                    });
                } catch (itemErr) {
                    labeledResults.push({ id: ev.id, suggestedRegion: null, suggestedCategory: null, embedding: null });
                }
            }
