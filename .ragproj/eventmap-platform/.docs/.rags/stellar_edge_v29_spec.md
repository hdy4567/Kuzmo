...
- **`navigation.js` (Feature)**: `setSubFilter` 이벤트 발생 시 태그 유무를 판단하여 시각화 엔진 호출.
- **`VisualizerCenter.js` (View-Map)**: 시각화의 중추. 디밍 제어, 마커 하이라이트 트리거, 연결선 렌더링 요청.
- **`map_renderer.js` (View-Map)**: 실제 Leaflet Polyline 레이어에 애니메이션 클래스(`rag-edge-glow`)를 주입하여 렌더링.
- **`map_ui.css` (Style)**: GPU 가속 기반의 `filter`, `transform`, `keyframes` 정의.
