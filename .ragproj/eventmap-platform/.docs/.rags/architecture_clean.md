...
```text
src/
 ├── core/ (Domain/Technology Layer)
 │    ├── engine/        # AI, 동기화, 데이터 처리 핵심 알고리즘
 │    ├── models/        # KZM 파일 포맷 및 데이터 스키마 정의
 │    └── store/         # 플랫폼 독립적인 원자적 상태 관리
 ├── infra/ (Infrastructure/Environment Layer)
 │    ├── network/       # Fetch, WebSocket 연동 (Mock 가능)
 │    └── persistence/   # IndexedDB, FileSystemHandle 연동
 └── view/ (Presentation/UI Layer)
      ├── layouts/       # Web(45% View), Mobile(Full), Desktop(Dock)
      ├── components/    # 디자인 시스템 기반 아토믹 요소
      └── handlers/      # UI 이벤트와 core/engine을 연결하는 어댑터
```
